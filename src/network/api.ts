...
export const addPayment = async (payload: {
  ...
  form.append("bill_id", payload.bill_id);
...
export const getBills = async () => {
  const res = await axiosInstance.get("/fetch-bill");
  return res.data;
};
...
export async function payPayment(payload: {
  payment_id: string | number;
  amount: any;
}) {
  // axios example:
  // return (await axios.post("/payments/pay", payload)).data;
  // fetch example:
  // const res = await fetch(BASE_URL + "/payments/pay", { method:"POST", headers:{...}, body: JSON.stringify(payload) });
